...
def make_args(**kwargs):
    defaults = dict(
        target='localhost:3270', sleep=0.5, clobber=False,
        user='uid1', password='pass1', appuser=None, apppassword=None,
        config=FIXTURE, debug=False, visable=False, overtype=False,
        env_switch=False, bulk_auth=False, check_cics=False,
        check_app=False, check_user=False, logmein=False,
        changepass=False, populate_cics=False, populate_apps=False,
        populate_users=False, bulk_auth_create=False, excel=False,
        gen_excel_testing=False, manual_inport=False, manual_export=False,
        file_input=None, file_output=None, que=False, mq=False,
        destructive=False, department=False, cemt_trans=False,
    )
...
class TestMainFrameBadAppCodes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mf.credentials = {
            'appcredentials': {'user': 'uid1', 'password': 'pass1'}
        }
...
class TestMainFrameCheckScreenForString(unittest.TestCase):
    ...
    def test_string_found(self):
        self.mf.em.screen_get = MagicMock(return_value=[
