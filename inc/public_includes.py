...
def read_xml(configfile, first_level_element_to_find):
    ###
    # reads the xml, returns a list of dictionaries
    # matching the requested element to find.
    ###
...
def process_mq_results_into_excel(
    ...
    if pre_existing_application_code is None:
        ##
        # No match, new application code.
        # Insert application code and response type under user
        ##
...
def screen(text, type="clear", level=0):
    ...
    print(
        color + tab_count + prepend + str(text) + (bcolors.ENDC if color != "" else "")
    )
